...
# Name-search snippet (plain strings interpolated into the f-string below; their
# braces are values, not re-parsed). JS goes inside the renderer's <script>.
def _search_css() -> str:
    return (
...
# "About this data" modal — same look/behaviour as the rest of the
# facetwork-maps gallery. Plain strings (braces are values, not re-parsed by the
# f-string below).
def _modal_css() -> str:
    return (
...
def _about_html(year: int) -> str:
    """Modal body — reuses the map's existing description + source/provenance text."""
    repo = escape(FFL_URL.split("/blob/")[0])
    return (
        f'<p style="margin:0 0 10px">Armed-conflict indicators by country ({year}), shaded '
        "<b>dark = worse</b>. Pick a metric in the control panel; click a country for all "
        "values.</p>"
        '<p style="margin:0 0 10px"><b>Sources:</b> UCDP (events, deaths, civilian targeting, '
        "actors), UNHCR (displaced), IDMC (new displacements), IPC (food insecurity).</p>"
        '<p style="margin:0">Generated by Facetwork workflow '
        "<code>conflict.workflows.BuildConflictWorldMap</code> &middot; "
